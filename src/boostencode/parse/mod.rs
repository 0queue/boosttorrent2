...
pub fn parse_val(bytes: &mut Vec<u8>) -> Result<Value, BDecodeError> {
    match bytes[0] as char {
        'i' => parse_integer(bytes),
        'l' => parse_list(bytes),
        'd' => parse_dict(bytes),
        '0'...'9' => parse_bstring(bytes),
        _ => return Err(BDecodeError::InvalidValue)
    }
}

// assured of a bstring, we take it off the stack of bytes and return it
fn parse_bstring(bytes: &mut Vec<u8>) -> Result<Value, BDecodeError> {
    let len = parse_integer_literal(bytes)?;
    if bytes.remove(0) as char != ':' {
        return Err(BDecodeError::InvalidString);
    }
...
fn parse_integer(bytes: &mut Vec<u8>) -> Result<Value, BDecodeError> {
    if bytes.remove(0) as char != 'i' {
        return Err(BDecodeError::InvalidInteger);
    }
...
fn parse_list(bytes: &mut Vec<u8>) -> Result<Value, BDecodeError> {
    let mut list = Vec::new();
    if bytes.remove(0) as char != 'l' {
        return Err(BDecodeError::InvalidList);
    }
...
fn parse_dict(bytes: &mut Vec<u8>) -> Result<Value, BDecodeError> {
    let mut map = HashMap::new();

    if bytes.remove(0) as char != 'd' {
        return Err(BDecodeError::InvalidDict);
    }
...
fn parse_integer_literal(bytes: &mut Vec<u8>) -> Result<usize, BDecodeError> {
    let mut num = String::new();
    while bytes.len() > 0 && bytes[0] as char >= '0' && bytes[0] as char <= '9' {
        num.push(bytes.remove(0) as char);
    }

    usize::from_str(num.as_str()).map_err(|_| BDecodeError::InvalidInteger)
}